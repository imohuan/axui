...
const toggle = (key: ToggleKey) => {
  props.settings[key] = !props.settings[key]
}

const toggleNotify = (key: ToggleKey, msgOn: string, msgOff: string, type?: string) => {
  toggle(key)
  emit('notify', props.settings[key] ? msgOn : msgOff, type)
}
</script>
...
<template>
  <div class="space-y-ax-lg max-w-2xl">
    ...
    <section class="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden pro-shadow">
      ...
      <div class="p-ax-md space-y-0.5 divide-y divide-outline-variant/40">
        ...
        <div class="flex items-center justify-between py-3">
          ...
          <AxInput v-model="settings.consoleName" size="sm" placeholder="Axiom Console" class="w-44" />
        </div>
        <div class="flex items-center justify-between py-3">
          <div>
    ...
    <section class="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden pro-shadow">
      ...
      <div class="p-ax-md space-y-ax-md">
        <div class="space-y-ax-xs">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="font-body-sm text-[11px] text-secondary mt-0.5">设置系统允许占用的最大计算资源百分比</p>
            </div>
        ...
        <div class="flex items-center justify-between py-1 border-t border-outline-variant/40 pt-3">
          <div>
        ...
        <div class="flex items-center justify-between border-t border-outline-variant/40 pt-3">
          ...
          <button
            ...
            @click="toggleNotify('hwAccel', '硬件加速已启用', '硬件加速已关闭', 'info')"
          >
            <span
      ...
      <div class="p-ax-md space-y-ax-md divide-y divide-outline-variant/40">
        ...
        <div class="py-3">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="font-body-sm text-[11px] text-secondary mt-0.5">无操作自动注销的等待时间（分钟）</p>
            </div>
        ...
        <div class="flex items-center justify-between py-3">
          ...
          <button
            ...
            class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer items-center rounded-full p-0.5 transition-colors duration-200"
            @click="toggleNotify('twoFactor', '双因素认证已启用', '双因素认证已关闭，请注意安全风险', settings.twoFactor ? 'success' : 'error')"
          >
            <span
    ...
    <section class="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden pro-shadow">
      ...
      <div class="p-ax-md divide-y divide-outline-variant/40">
        <div v-for="notif in notifSettings" :key="notif.key" class="flex items-center justify-between py-3">
          <div>
      ...
      <div class="p-ax-md space-y-ax-md divide-y divide-outline-variant/40">
        ...
        <div class="flex items-center justify-between pt-3">
          ...
          <button
            ...
            class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer items-center rounded-full p-0.5 transition-colors duration-200"
            @click="toggleNotify('debugMode', '调试模式已开启，控制台将输出详细日志', '调试模式已关闭', 'secondary')"
          >
            <span
        ...
        <div class="pt-3 flex justify-end gap-ax-sm">
          <AxButton variant="outline" @click="reset">重置为默认值</AxButton>
          <AxButton icon="save" @click="save">保存所有设置</AxButton>
        </div>
      </div>
    </section>
  </div>
