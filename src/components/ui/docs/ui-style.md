...
| `AxSwitch` | 开关组件，支持 v-model、disabled；轨道 `h-5 w-9`，滑块 `h-4 w-4` |
...
`AxSwitch` 是独立的开关组件，也可通过 `AxPropPanel` type=`switch` 在属性面板中使用。
...
```vue
<!-- 独立使用 -->
<AxSwitch v-model="checked" />
<AxSwitch v-model="checked" disabled />

<!-- 通过 AxPropPanel -->
<AxPropPanel v-model="props" :schema="[{
  key: 'checked', label: '开关', type: 'switch'
}]" />
```


尺寸：轨道 `h-5 w-9`，滑块 `h-4 w-4`。
