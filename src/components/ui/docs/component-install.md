...
自研 Vue 3 组件库（`Ax*` 基础组件 + `functional` 功能模块）。本文档只说明**如何接入**；视觉 token、组件尺寸与交互规范见 [`ui-style.md`](./ui-style.md)，接入时按该文档同步 `style.css` 与 `main.ts` 字体引入即可。
...
本节将链接并摘要 [`ui-style.md`](./ui-style.md) 中的设计原则、token 表、组件尺寸与动效约定，便于与实现代码对照。当前仅完成安装链路，风格说明暂不展开。
